...
// Action: 获取框架属性列表.
import AttrList = require("./app-config/attr-list");
const attrList: string[] = AttrList.AttrList();  // 框架属性列表.
...
// Definition: 扫描控制器节点.
export function scanCtrlDoms() : void {
    ...
    for (let i = 0, length = lcControllers.length; i < length; i++) {
        ...
        const domsInCtrl = getChildDoms(lcControllers[i]);  // 当前控制器下的所有元素.
        
        // 遍历所有子元素并检查其属性.
        for (let i = 0, length = domsInCtrl.length; i < length; i++) {
            const attrs = domsInCtrl[i].attributes;  // 当前子元素的所有属性.
            
            // 遍历当前子元素的所有属性.
            for (let index in attrs) {
                if (!attrs.hasOwnPorperty(index)) { return; }
                
                const attrName = attrs[index].name;  // 当前扫描的属性名称.
                const attrValue = attrs[index].value;  // 当前扫描的属性的值.
...
function getChildDoms (dom) {
    ...
    for (let i = 0, length = dom.children.length; i < length; i++) {
        const children = dom.children[i];  // 当前扫描的后代节点.
